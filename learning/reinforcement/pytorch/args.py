...
def get_ddpg_args_train():
    ...
    parser.add_argument(
        "--start_timesteps", default=10, type=int
    )  # How many time steps purely random policy is run for
    parser.add_argument("--eval_freq", default=2, type=float)  # How often (time steps) we evaluate
    parser.add_argument("--max_timesteps", default=10, type=float)  # Max time steps to run environment for
